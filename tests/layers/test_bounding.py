...
def test_relu_bounding(config, name_to_index, input_tensor, statistics):
    bounding = NormalizedReluBounding(variables=config.variables, name_to_index=name_to_index, min_val= [2.0, 2.0], normalizer= "mean-std", statistics= statistics)
    output = bounding(input_tensor.clone())
    breakpoint()
